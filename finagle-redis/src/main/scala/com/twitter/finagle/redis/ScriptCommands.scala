...
trait Scripts {
   ...
   */
  def eval(script: ChannelBuffer, keys: Seq[ChannelBuffer], argv: Seq[ChannelBuffer]): Future[Reply] =
   ...
   * @return redis-server [[Reply]]
   */
  def evalSha(sha: ChannelBuffer, keys: Seq[ChannelBuffer], argv: Seq[ChannelBuffer]): Future[Reply] =
   ...
   * @return redis-server [[Reply]]
   */
  def evalSha(sha: ChannelBuffer, script: ChannelBuffer, keys: Seq[ChannelBuffer], argv: Seq[ChannelBuffer]): Future[Reply] = {
...
object Scripts {
  ...
  class CastableReply(reply: Reply) {
    def cast[T]()(implicit ev: ReplyConverter[T]): T =
      ev.cast(reply)
  }
  implicit def replyToCastable(reply: Reply) =
    new CastableReply(reply)
  ...
  object ReplyConverter {
    implicit val unit = new ReplyConverter[Unit] {
      override def cast(reply: Reply) = reply match {
    ...
    implicit val buffer = new ReplyConverter[ChannelBuffer] {
      override def cast(reply: Reply) = reply match {
        case BulkReply(message) => message
        case EmptyBulkReply() => ChannelBuffers.EMPTY_BUFFER
        case r => throw new ReplyCastFailure("Error casting " + r + " to ChannelBuffer")
      }
    }

    implicit val buffers = new ReplyConverter[Seq[ChannelBuffer]] {
      override def cast(reply: Reply) = reply match {
        case MBulkReply(messages) => ReplyFormat.toChannelBuffers(messages)
        case EmptyMBulkReply() => Nil
        case r => throw new ReplyCastFailure("Error casting " + r + " to Seq[ChannelBuffer]")
      }
    }
  }
}
