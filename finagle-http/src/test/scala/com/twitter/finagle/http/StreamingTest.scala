...
import com.twitter.finagle.{Service, ServiceFactory, ServiceProxy}
import com.twitter.finagle.builder.{ClientBuilder, ServerBuilder}
import com.twitter.finagle.{
  ClientCodecConfig, Codec, CodecFactory, ServerCodecConfig,
...
class StreamingTest extends FunSuite with Eventually {
  ...
  test("client: request stream fails on write") (new ClientCtx {
    ...
    await(req.writer.write(buf))
    intercept[Reader.ReaderDiscarded] { await(req.writer.write(buf)) }
