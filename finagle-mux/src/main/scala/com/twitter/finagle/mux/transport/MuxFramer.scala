...
private[finagle] object MuxFramer {
   ...
   */
  object Header {
    val KeyBuf: ChannelBuffer = wrappedBuffer("mux-framer".getBytes(Charsets.Utf8))
    val DefaultWindowBuf: ChannelBuffer = encodeWindow(DefaultFrameSize)

    /**
     * Returns a header value with the given `window` size.
     */
    def encodeWindow(window: Int): ChannelBuffer = {
    ...
    /**
     * Extracts a window size from the `cb`.
     */
    def decodeWindow(cb: ChannelBuffer): Int = cb.readInt()
  }
   ...
   * fragments writes into `window` sized payloads and defrags reads
   * into mux [[Message]]s.
   */
  def apply(
    trans: Transport[ChannelBuffer, ChannelBuffer],
    window: Int = DefaultFrameSize
     ...
     * be <= `window`. Note, this should not be iterated over on more than one
     * thread at a time.
     */
    private[this] def fragment(msg: Message): Iterator[ChannelBuffer] =
      new Iterator[ChannelBuffer] {
        ...
        def next(): ChannelBuffer = {
          ...
          val len = math.min(readable - read, window)
    ...
    def write(msg: Message): Future[Unit] = msg match {
      case m@(_: Message.Tdispatch | _: Message.Rdispatch)
        if m.buf.readableBytes > window =>
          val fragments = fragment(m)
          // write the head, enqueue the tail
