...
class JavaClientBase(underlying: Client) extends JavaClient {
  ...
  override def gets(key: String): Future[ResultWithCAS] = underlying.gets(key).map(_.map {
      case (b1, b2) => new ResultWithCAS(b1, b2)
    }.getOrElse(null))
  override def get(keys: juList[String]): Future[juMap[String, Buf]] = underlying.get(keys.asScala).map(_.asJava)
  override def gets(keys: juList[String]): Future[juMap[String, ResultWithCAS]] = underlying.gets(keys.asScala)
    .map(_.mapValues {
      case (b1, b2) => new ResultWithCAS(b1, b2)
    }.toMap.asJava)

  override def set(key: String, value: Buf): Future[Void] = underlying.set(key, value).map(_ => null)
  ...
  override def add(key: String, flags: Int, expiry: Time, value: Buf) = underlying.add(key, flags, expiry, value)
  ...
  override def replace(key: String, flags: Int, expiry: Time, value: Buf) = underlying.replace(key, flags, expiry, value)

  override def cas(key: String, value: Buf, casUnique: Buf) = underlying.checkAndSet(key, value, casUnique).map(_.replaced)
