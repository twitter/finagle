...
import com.twitter.finagle.redis.Scripts._
...
// Test scripting features of redis Client
...
class ScriptClientIntegrationSuite extends RedisClientTest {
  ...
  def testScriptAndSha[T, X](
    ...
    cast: Reply => T,
    testCase: (String, Seq[ChannelBuffer], Client => (ChannelBuffer, Seq[ChannelBuffer], Seq[ChannelBuffer]) => Future[T]) => X
  ) = {
