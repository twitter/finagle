...
class HttpMuxerTest extends FunSuite {
  ...
  test("prefix matching is handled correctly") {
    assert(Response(Await.result(muxService(Request("/foo/<a>")))).contentString === percentEncode)
    ...
    assert(Response(Await.result(muxService(Request("/foo//bar/<a>")))).contentString === fooBarPrefix)
  ...
  test("""special cases "" and "/" are handled correctly""") {
    ...
    assert(Response(Await.result(slashMux(Request("")))).status === HttpResponseStatus.NOT_FOUND)
