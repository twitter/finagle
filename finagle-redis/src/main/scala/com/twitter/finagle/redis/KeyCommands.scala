...
trait Keys { self: BaseClient =>
  ...
  def del(keys: Seq[ChannelBuffer]): Future[JLong] =
    dels(keys.map(ChannelBufferBuf.Owned(_)))
  ...
  def dump(key: ChannelBuffer): Future[Option[ChannelBuffer]] =
    dump(ChannelBufferBuf.Owned(key)).map { response =>
      response.map(ChannelBufferBuf.Owned.extract)
    }
  ...
  def exists(key: ChannelBuffer): Future[JBoolean] =
    exists(ChannelBufferBuf.Owned(key))
  ...
  def expire(key: ChannelBuffer, ttl: JLong): Future[JBoolean] =
    expire(ChannelBufferBuf.Owned(key), ttl)
  ...
  def expireAt(key: ChannelBuffer, ttl: JLong): Future[JBoolean] =
    expireAt(ChannelBufferBuf.Owned(key), ttl)
  ...
  def keys(pattern: ChannelBuffer): Future[Seq[ChannelBuffer]] =
    keys(ChannelBufferBuf.Owned(pattern)).map { response =>
      response.map(ChannelBufferBuf.Owned.extract)
    }
  ...
  @deprecated("remove netty3 types from public API", "2016-03-24")
   def move(key: ChannelBuffer, db: ChannelBuffer): Future[JBoolean] =
     move(ChannelBufferBuf.Owned(key), ChannelBufferBuf.Owned(db))
  ...
  def pExpire(key: ChannelBuffer, milliseconds: JLong): Future[JBoolean] =
    pExpire(ChannelBufferBuf.Owned(key), milliseconds)
  ...
  def pExpireAt(key: ChannelBuffer, timestamp: JLong): Future[JBoolean] =
    pExpireAt(ChannelBufferBuf.Owned(key), timestamp)
  ...
  def pTtl(key: ChannelBuffer): Future[Option[JLong]] =
    pTtl(ChannelBufferBuf.Owned(key))
   ...
   */
  @deprecated("remove netty3 types from public API", "2016-03-24")
  def scan(cursor: JLong, count: Option[JLong], pattern: Option[ChannelBuffer]): Future[Seq[ChannelBuffer]] =
    scans(cursor, count, pattern.map(ChannelBufferBuf.Owned(_))).map { response =>
      response.map(ChannelBufferBuf.Owned.extract)
    }
  ...
  def ttl(key: ChannelBuffer): Future[Option[JLong]] =
    ttl(ChannelBufferBuf.Owned(key))
