...
class ServerBuilderSpec extends SpecificationWithJUnit with IntegrationBase with Mockito {
  "ServerBuilder" should {
    ...
    "build server that disposes of resources used" in {
      val address = mock[SocketAddress]
    ...
    "build managed server that disposes of resources used" in {
      val address = mock[SocketAddress]
      val tracer = mock[Tracer]

      val mServer = serverBuilder
        .bindTo(address)
        .tracerFactory { cn => cn.onClose { tracer.release() }; tracer }
        .buildManaged(service)

      mServer foreach { _ =>
        there was no(tracer).release()
      }
      there was one(tracer).release()
    }
