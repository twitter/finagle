...
import com.twitter.finagle.mux.exp.FailureDetector.ThresholdConfig
import com.twitter.finagle.mux.{NullFailureDetector, ThresholdFailureDetector}
...
class FailureDetectorTest extends FunSuite
{
  ...
  test("explicit threshold used") {
    assert(FailureDetector(new ThresholdConfig, ping, close, statsReceiver).isInstanceOf[ThresholdFailureDetector])
  }
}
