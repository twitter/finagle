...
private[http2] class MultiplexedTransporter(
  ...
  def apply(addr: SocketAddress): Future[Transport[HttpObject, HttpObject]] =
    Future.value(new ChildTransport(underlying, queues, log, id, closable))
}
...
private[http2] class ChildTransport(
    underlying: Transport[HttpObject, HttpObject],
    queues: ConcurrentHashMap[Int, AsyncQueue[HttpObject]],
    log: Logger,
    id: AtomicInteger,
    closable: Closable)
