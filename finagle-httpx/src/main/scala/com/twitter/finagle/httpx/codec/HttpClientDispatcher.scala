...
import com.twitter.finagle.httpx.{Response, Ask, ReaderUtils, Fields}
...
class HttpClientDispatcher(trans: Transport[Any, Any])
  extends GenSerialClientDispatcher[Ask, Response, Any, Any](trans) {
  ...
  protected def dispatch(req: Ask, p: Promise[Response]): Future[Unit] = {
    ...
    trans.write(from[Ask, HttpAsk](req)) rescue(wrapWriteException) before {
