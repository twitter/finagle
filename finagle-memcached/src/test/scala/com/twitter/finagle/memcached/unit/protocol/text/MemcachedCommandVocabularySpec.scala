...
import text.{MemcachedResponseVocabulary, MemcachedCommandVocabulary}
import com.twitter.conversions.time._
import com.twitter.util.Time

class MemcachedCommandVocabularySpec extends Specification {
  "MemcachedCommandVocabulary" should {
    ...
    "parse storage commands" in {
      Time.withCurrentTimeFrozen { tc =>
        val buffer = "bar"
        parser.parseStorageCommand(Seq("add",     "foo", "1", "2", "3"), buffer) mustEqual Add("foo", 1, 2.seconds.fromNow, buffer)
        parser.parseStorageCommand(Seq("set",     "foo", "1", "2", "3"), buffer) mustEqual Set("foo", 1, 2.seconds.fromNow, buffer)
        parser.parseStorageCommand(Seq("replace", "foo", "1", "2", "3"), buffer) mustEqual Replace("foo", 1, 2.seconds.fromNow, buffer)
        parser.parseStorageCommand(Seq("append",  "foo", "1", "2", "3"), buffer) mustEqual Append("foo", 1, 2.seconds.fromNow, buffer)
        parser.parseStorageCommand(Seq("prepend", "foo", "1", "2", "3"), buffer) mustEqual Prepend("foo", 1, 2.seconds.fromNow, buffer)
      }
    }
  }
