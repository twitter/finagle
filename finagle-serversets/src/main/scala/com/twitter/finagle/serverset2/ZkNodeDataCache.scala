...
private[serverset2] abstract class ZkNodeDataCache[Entity](
  ...
  protected[this] def loadEntity(path: String): Future[Seq[Entity]] =
    zkSession().immutableDataOf(clusterPath + "/" + path).map {
...
private[serverset2] class ZkEntryCache(
    ...
    clusterPath: String,
    statsReceiver: StatsReceiver)
  extends ZkNodeDataCache[Entry](zkSession, clusterPath, "Entry", statsReceiver) {
...
private[serverset2] class ZkVectorCache(
    ...
    clusterPath: String,
    statsReceiver: StatsReceiver)
  extends ZkNodeDataCache[Vector](zkSession, clusterPath, "Vector", statsReceiver) {
